...
import { FileUploadModal } from "./components/ingestion/FileUploadModal";
...
export const VSCodeOfficeStudio: React.FC<VSCodeOfficeStudioProps> = ({
  ...
  const [currentView, setCurrentView] = useState<"studio" | "history" | "chat">("studio");
      ...
      for (let i = 0; i < items.length; i++) {
        if (items[i].kind === "file") {
          ...
          if (file) {
            e.preventDefault();
            void upload(file);
            notify(`Imported ${file.name} from clipboard.`);
            break;
          }
        }
      }
  ...
  return (
    ...
    >
      ...
      {currentView === "history" ? (
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", height: "calc(100vh - 35px)" }}>
      ...
      <FileUploadModal
        ...
        onLoadDirectly={(file) => void upload(file)}
        onUploadSuccess={(jobId) => {
          setCurrentView("history");
          notify(`Conversion job started: ${jobId}`);
        }}
      />
      {notice && (
        <div
